...
\noindent
\textbf{Note:} Approach 2 (Short Notebook 2+Rolling) extends this base feature set with over 100 additional advanced features, 
...
Given the need to train 121 material-specific models, computational efficiency was a critical factor in our final selection.
...
For the advanced ensemble approach (Short Notebook 2+Rolling), we combined \textbf{CatBoost and LightGBM} with Optuna-tuned hyperparameters 
...
% ============================================================================
\section{Modeling Approach 2: Advanced Ensemble (Short Notebook 2+Rolling)}
% ============================================================================

This section describes our second forecasting methodology, implemented in Short Notebook 2+Rolling.
...
Beyond the 17 base features used in the material-specific approach, we engineered \textbf{over 100 additional advanced features} 
capturing higher-order patterns, cross-feature interactions, and sophisticated statistical properties.
...
Table~\ref{tab:advanced_features} summarizes the complete feature set used in Short Notebook 2+Rolling.
This represents a significant expansion from the 17 base features in Approach 1, adding over 100 advanced features 
to capture higher-order patterns, interactions, and material-level statistics.

\begin{table}[H]
\centering
\caption{Advanced Feature Categories for Approach 2 (Total: 120+ features)}
...
Rolling windows (8 windows × 6 stats: sum, mean, std, max, count, EWM) & 48 \\
...
This iterative testing of post-processing strategies was crucial for optimizing the final score. 
After analyzing the validation performance and the resulting prediction statistics, the file \texttt{submission\_S0.90\_T15000kg\_xxxxxx.csv} was selected as the definitive submission for this approach.

This choice represents the optimal tradeoff for the competition's Quantile Error metric (q=0.2). The reasoning is twofold:

\begin{itemize}
    \item \textbf{Shrinkage Factor (0.90):} A conservative global shrinkage factor of 0.90 was applied to uniformly reduce all predictions, mitigating the heavy penalty for over-prediction.
    \item \textbf{Zeroing Threshold (15,000kg):} An aggressive 15,000kg threshold was implemented, setting all cumulative predictions below this value to zero. This step was vital for aligning with the conservative nature of the q=0.2 metric, as it significantly increased the number of zero predictions and eliminated low-confidence positive values that were likely to be penalized.
\end{itemize}
...
\textbf{Approach 2 (Short Notebook 2+Rolling)} using an advanced ensemble with extensive feature engineering.
...
This approach trained \textbf{121 individual LightGBM models}, one per active material, using recursive day-by-day forecasting with material-specific adaptive shrinkage.
...
\subsubsection{Approach 2: Ensemble with Advanced Features (Short Notebook 2+Rolling)}

This approach combined \textbf{CatBoost and LightGBM} with extensive feature engineering (120+ features vs. 17) 
and Optuna hyperparameter optimization, trained on synthetic forecasting tasks spanning 2005--2023.

\begin{table}[H]
\centering
\caption{Short Notebook 2+Rolling - Performance Summary}
...
\quad CatBoost (Optuna-tuned) Quantile Loss & 12,348 \\
\quad LightGBM (Optuna-tuned) Quantile Loss & 11,342 \\
\quad Ensemble (60/40 blend) & \textbf{11,945} (estimated) \\
\midrule
\textbf{Feature Engineering} & \\
\quad Total features & 120+ \\
\quad Advanced features added & 100+ \\
\quad Key innovations & Fourier, target encoding, interactions, higher-order stats \\
\midrule
\textbf{Model Configuration} & \\
...
\midrule
Approach 1 (Material-specific) & 5663 & \texttt{submission\_best.csv} \\
Approach 2 (Ensemble) & 5406 & \texttt{submission\_S0.90\_T15000kg\_20251108\_200603.csv} \\
\bottomrule
\end{tabular}
\end{table}
...
Approach 1 requires more training time due to 121 individual models but has minimal tuning overhead. 
...
Our first approach trained 121 individual LightGBM models with material-specific adaptive shrinkage factors. 
...
This methodology achieved validation Quantile Loss of \textbf{12,348 for CatBoost} and \textbf{11,342 for LightGBM} 
